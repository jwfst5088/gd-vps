...
impl Default for RuleBotPlugin {
    fn default() -> Self {
        let params = RuleBotParams::load("params.json").unwrap_or_else(|_| {
            eprintln!("[rule-bot] No params.json found, using default balanced profile");
            RuleBotParams::default_balanced()
        });
        eprintln!("[rule-bot] Loaded params: {:?}", params);

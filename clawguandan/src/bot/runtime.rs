...
const BOT_WAIT4MYTURN_TIMEOUT_MS: u64 = 25_000;
...
pub fn run_bot_subprocess(opts: BotRunOptions, plugin: Arc<dyn BotPlugin>) -> Result<(), String> {
    ...
    // When --players 0 is explicitly passed, discover existing bot players
    // instead of joining new ones. This allows the bot process to manage
    // bots that were already registered via the API (e.g., from the UI).
    let discover_existing = opts.players == Some(0);

    if discover_existing {
        // Discover existing bot-type players from the snapshot
        for (seat_key, seat_val) in seats.iter() {
            if let Some(player_type) = seat_val.get("playerType").and_then(|x| x.as_str()) {
                if player_type == "bot" {
                    if let Some(pid) = seat_val.get("playerId").and_then(|x| x.as_str()) {
                        let name = seat_val
                            .get("playerName")
                            .and_then(|x| x.as_str())
                            .unwrap_or("bot");
                        let ready = seat_val
                            ...
                            .unwrap_or(false);
                        println!(
                            "[{plugin_id}][I][bot:discovered] table={table_id} seat={seat_key} player={pid} name={name} ready={ready}"
                        );
                        pids.push(pid.to_string());
                        display_names.push(name.to_string());
                        // Bots are auto-readied by the server when they join,
                        // so we don't need to call `play ready` here.
                    }
                }
            }
        }
        if pids.is_empty() {
            return Err("--players 0 but no existing bot players found in table".into());
        }
        println!(
            "[{plugin_id}][I][bot:discovered] table={table_id} count={count}",
            count = pids.len()
        );
    } else {
        ...
        display_names = match plugin.name_policy().join_display_names(&join_ctx) {
    ...
    handles.push(thread::spawn(move || loop {
        ...
        let out = match run_cli_command(&bin_obs, &argv) {
            ...
            Err(e) => {
                ...
                let msg = e.to_string();
                if msg.contains("error sending request") || msg.contains("connection") || msg.contains("temporarily unavailable") || msg.contains("os error 35") {

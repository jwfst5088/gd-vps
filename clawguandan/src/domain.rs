...
pub struct TableRuntimeState {
    ...
    /// Game winner team ID (set when the game ends).
    pub game_winner_team_id: Option<TeamId>,
...
impl TableRuntimeState {
    ...
    fn materialize_hand_json(&self, g: &TableGameState) -> serde_json::Value {
        let Some(hand) = g.hand.as_ref() else {
            eprintln!(
                "WARN: materialize_hand_json: hand is None. table={}, phase={:?}, status={:?}",
                self.table_id, g.phase, self.status
            );
            return json!(null);
        };
    ...
    fn is_current_actor_bot(&self, seat: Option<Seat>) -> bool {
        seat.and_then(|s| self.seats.get(&s).and_then(|p| p.as_ref()))
            .is_some_and(|p| matches!(p.player_type, PlayerType::Bot))
    }

    fn bot_deadline(&self, seat: Option<Seat>) -> Option<String> {
        if self.is_current_actor_bot(seat) {
            Some(iso_timestamp_after_secs(BOT_TURN_DEADLINE_SECS))
        } else {
            Some(iso_timestamp())
        }
    }
        ...
        match g.phase {
            GamePhase::Tribute => {
                ...
                Expect {
                    ...
                    deadline_at: self.bot_deadline(actor),
                }
            }
            GamePhase::Exchange => {
            ...
            GamePhase::Playing => {
                ...
                Expect {
                    ...
                    deadline_at: self.bot_deadline(Some(g.turn_seat)),
                }
            }
            GamePhase::Scoring | GamePhase::Dealing | GamePhase::TableSetup => Expect {
                ...
                legal_actions: vec![],
                deadline_at: Some(iso_timestamp()),
            },
    ...
    pub fn to_table_state(&self) -> TableState {
        ...
        TableState {
            ...
            scoreboard: Scoreboard {
                finishing_order: self.last_finishing_order.iter().map(|s| s.as_str().to_string()).collect(),
                game_winner_team_id: self.game_winner_team_id.map(|t| t.as_str().to_string()),
                ..Scoreboard::default()
            },
        }
    }
...
/// Seconds before a bot must act or be auto-passed.
pub const BOT_TURN_DEADLINE_SECS: u64 = 30;

/// ISO timestamp for a point in time `secs` seconds from now.
pub fn iso_timestamp_after_secs(secs: u64) -> String {

...
impl CombinationParser {
    pub fn parse(
        ...
        if (4..=10).contains(&n) && all_same_rank(&resolved) {
            // level_order_value gives level card 14, A=13, K=12, ..., 3=2, 2=1.
            // This correctly makes level-card bombs the biggest among same-size bombs.
        ...
        match n {
            ...
            5 => {
                ...
                } else if is_full_house(&resolved, ctx)? {
...
fn is_full_house(cards: &[Card], ctx: RuleContext) -> Result<bool, String> {
    if cards.len() != 5 {
        return Ok(false);
    }
    if cards.iter().any(|c| c.suit == Suit::Joker) {
    ...
    if !(counts.values().any(|&n| n == 3) && counts.values().any(|&n| n == 2)) {
        return Ok(false);
    }
    Ok(true)
}

...
impl GameEngine {
    ...
    pub fn start_next_hand_with_tribute(
        ...
        // Build tribute plan with fallback: if it fails, treat as canceled tribute
        let tribute = match build_tribute_plan(&hand, declarer, last_finishing_order) {
            Ok(t) => t,
            Err(e) => {
                eprintln!(
                    "WARN: build_tribute_plan failed for table {}: {}. Falling back to canceled tribute.",
                    state.table_id, e
                );
                TributeState {
                    pairs: Vec::new(),
                    canceled: true,
                    opening_lead_candidates: vec![first_drawer],
                }
            }
        };
...
impl GameEngine {
    ...
    fn apply_exchange(
        ...
        let level_rank = hand.hand_level.to_rank();
        if return_rank == level_rank {
            return Err("cannot return level card (级牌)".into());
        }
        remove_cards_from_hand(&mut hand.hands, actor, std::slice::from_ref(&card))?;
    ...
    fn apply_playing(
        ...
        match action {
            PlayerAction::Play {
                ...
                if hand.remaining_count(actor) == 0 && !hand.finishing_order.contains(&actor) {
                    ...
                    if hand.finishing_order.len() == 1 {
                        state.winner_team = Some(seat_team(actor));
                    }
                }
            ...
            PlayerAction::Pass => {
                ...
                if hand.trick.consecutive_passes >= required_passes {
                    // Trick ends; 接风规则：队友完牌且没被管上时同门接风，被管上则谁管上谁出牌
                    let last_play = hand
                        .trick
                        .last_play_seat
                        .expect("trick ended without a play");
                    let lead = hand
                        .finishing_order
                        .last()
                        .filter(|&&finisher| last_play == finisher)
                        .and_then(|&finisher| {
                            let p = partner(finisher);
                            (hand.remaining_count(p) > 0).then_some(p)
                        })
                        .unwrap_or(last_play);
        ...
        if is_team_empty(hand, TeamId::Ew) || is_team_empty(hand, TeamId::Sn) {
            ...
            // winner_team is set by first finisher above
        }

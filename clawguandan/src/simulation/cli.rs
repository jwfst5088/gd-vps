...
/// Retry counters for resource-exhausted errors (os error 35 on FreeBSD).
...
/// Retries on EAGAIN (os error 35, "Resource temporarily unavailable") up to `MAX_RETRIES` times.
pub fn run_cli_command(
    ...
    for attempt in 0..=MAX_RETRIES {
        match Command::new(bin).args(args).output() {
            ...
            Err(e) => {
                let is_resource_unavailable = e.raw_os_error() == Some(35);
                if is_resource_unavailable && attempt < MAX_RETRIES {
                    eprintln!(
                        "run_cli_command: resource temporarily unavailable (attempt {}/{}), retrying in {}ms",
                        attempt + 1,
                        MAX_RETRIES + 1,
                        RETRY_DELAY_MS
                    );
                ...
                return Err(CliRunError::Io(e));
            }
        }
    }
    Err(CliRunError::Io(last_err.unwrap()))
}

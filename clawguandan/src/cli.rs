...
use clawguandan::bot::plugins::{
    AdvancedBotPlugin, BeatItPlugin, LlmBotParams, LlmBotPlugin, RuleBotPlugin, resolve_join_model,
    verify_script_model,
};
...
pub(crate) enum Top {
    ...
    /// AI self-learning: optimize bot parameters via self-play
    Learn {
        /// Number of matches per evaluation (default: 100)
        #[arg(short = 'm', long, default_value = "100")]
        matches: u32,
        /// Number of optimization iterations (default: 50)
        #[arg(short = 'i', long, default_value = "50")]
        iterations: u32,
        /// Output path for optimized params JSON (default: advanced_params.json)
        #[arg(short = 'o', long, default_value = "advanced_params.json")]
        output: String,
    },
    /// AI learning from game logs: optimize bot parameters from recorded human vs bot matches
    LearnFromLogs {
...
pub(crate) enum BotCmd {
    ...
    /// Run advanced bots with hand tracking and probabilistic reasoning. Optionally target an existing table; otherwise create one.
    AdvancedBot {
        /// Optional existing table ID to join. If omitted, creates a fresh table.
        #[arg(short = 't', long)]
        table: Option<String>,
    ...
    match command {
        ...
        Top::Bot { cmd } => match cmd {
            ...
            BotCmd::AdvancedBot {
                ...
                hands,
                verbosity,
            } => simulate_advanced_bot_subprocess(table, rank, players, hands, verbosity),
            BotCmd::LlmBot {
                table,
                rank,
                players,
        ...
        Top::Learn {
            ...
            use clawguandan::learning::run_learning;
            run_learning(matches, iterations, &output)
        }
        Top::LearnFromLogs {
            log_file,
            iterations,
...
fn simulate_advanced_bot_subprocess(
    table: Option<String>,
    rank: Option<String>,
    players: Option<u8>,
    hands: Option<u32>,
    ...
    run_bot_subprocess(
        ...
        Arc::new(AdvancedBotPlugin::default()),
    )
}
